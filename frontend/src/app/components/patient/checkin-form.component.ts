...
@Component({
  ...
  template: `
    ...
    @if (questionIndex() === 1) {
      <div class="question-screen fade-in">
        ...
        <button class="continue-btn" (click)="confirmNeeds()" aria-label="Continue">
          →
        </button>
      </div>
    }
    ...
    @if (questionIndex() === 2) {
      <div class="question-screen fade-in">
        ...
        <div class="leave-buttons">
          <button class="leave-btn staying" (click)="selectLeave('staying')"
                  [attr.aria-label]="i18n.t('leaveStaying')">
            {{ i18n.t('leaveStaying') }}
          </button>
          <button class="leave-btn unsure" (click)="selectLeave('unsure')"
                  [attr.aria-label]="i18n.t('leaveUnsure')">
            {{ i18n.t('leaveUnsure') }}
          </button>
          <button class="leave-btn leaving" (click)="selectLeave('leaving')"
  ...
  styles: [`
    ...
    .discomfort-btn {
      flex: 1; min-height: 80px; min-width: 56px;
      display: flex; flex-direction: column;
      align-items: center; justify-content: center; gap: 0.25rem;
      border: 2px solid #0d47a1; border-radius: 14px;
      background: white; cursor: pointer;
      transition: background 0.15s, color 0.15s, transform 0.1s;
    }
    .discomfort-btn:active { transform: scale(0.95); }
    .discomfort-btn .emoji { font-size: 1.8rem; }
    .discomfort-btn .label { font-size: 0.9rem; font-weight: 700; color: #0d47a1; }
    ...
    .need-btn {
      width: 100%; min-height: 56px; padding: 0.75rem 1rem;
    ...
    .need-btn.selected {
      border-color: #0d47a1; background: #e3f2fd; color: #0d47a1;
    }
    .need-icon { font-size: 1.3rem; }
    .continue-btn {
      align-self: flex-end; width: 64px; height: 64px;
      border-radius: 50%; border: none;
      background: #0d47a1; color: white;
      font-size: 1.8rem; cursor: pointer;
      transition: transform 0.1s;
    }
    .continue-btn:active { transform: scale(0.93); }
    ...
    .leave-btn:active { transform: scale(0.97); opacity: 0.9; }
    .staying { background: #2e7d32; color: white; }
    .unsure  { background: #f57f17; color: white; }
    .leaving { background: #c62828; color: white; }

    /* Thank you */
    .thank-you { padding: 2rem 0; }
    .ty-icon { font-size: 3rem; }
    .thank-you h2 { font-size: 1.5rem; color: #2e7d32; margin: 0; }
    .thank-you p { color: #555; font-size: 1.1rem; margin: 0.5rem 0 0; }
    .returning { font-size: 0.9rem; color: #999; margin-top: 1rem !important; }
  `],
})
